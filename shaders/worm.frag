...
vec3 map(vec3 p)
{
    ...
    vec3 d1 = vec3(h1.x-0.15*(0.8+0.2*sin(200.0*h1.y)), ve + 0.000, h1.y);
    vec3 d2 = vec3(h2.x-0.15*(0.8+0.2*sin(200.0*h2.y)), ve + 0.015, h2.y);
    vec3 d3 = vec3(h3.x-0.15*(0.8+0.2*sin(200.0*h3.y)), ve + 0.030, h3.y);
...
{
    const vec2 e = vec2(1.0,-1.0)/256.0;
...
vec3 render(in vec3 ro, in vec3 rd, in float px)
{
    ...
    if(res.x < MaxDistance) {
        ...
        /// moving high light
        float fl = mod((0.5+cos(2.0+res.y*47.0))*u_time + res.y*7.0, 4.0)/4.0;
        col *= 2.5 - 1.5*smoothstep(0.02,0.04,abs(res.z-fl));
        /// MOD moving shadow in reverse direction
         float fl_rev = mod(-(0.5+cos(2.0+res.y*93.0))*u_time*2 + res.y*3.0, 4.0)/4.0;
         col *= 0.5 + 0.5*smoothstep(0.02,0.04,abs(res.z-fl_rev));

        /// attenuation
        col *= exp(-0.125/2*res.x); // all along the way
...
void main()
{
    ...
    vec3 col = render(ro, rd, 1.0/(u_fbsize.y*fl));
